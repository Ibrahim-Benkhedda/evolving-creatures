...
class Genome():
    """
    Genome class represents the genetic makeup of a creature.
    it provides methods to generate and manipulate the genome,
    which consists of an array of genes, where each gene being an array of floats
    """
    ...
    def get_random_gene(length):
        ...
        length - integer that is the length of the gene
        """
    ...
    def get_random_genome(gene_length, gene_count):
        """
        generates a random genome composed of a specified number of genes.

        gene_length - length of each gene in the genome.
    ...
    def get_gene_spec():
        """
        gets the specification of the gene and returns dictionary where,
        each key is a gene feature and each value is a
        dictionary containing the 'scale' and index 'ind' of the feature
        """
        gene_spec =  {
            "link-shape":{"scale":1}, 
            "link-length": {"scale":2},
    ...
    def get_gene_dict(gene, spec):
        """
        returns a dictionary representing the gene with scaled values
        """
        gene_dict = {}
        for key in spec:
            ind = spec[key]["ind"]
    ...
    def get_genome_dicts(genome, spec):
        """
        converts a genome (list of genes) into a list of dictionaries based on the gene specs
        and returns the list of dictionaries
        returns A list of dictionaries, each representing a gene
        

        genome - a genome list to convert.
        spec - The gene specification dictionary
    ...
    def expandLinks(parent_link, uniq_parent_name, flat_links, exp_links):
        """
        recursively expands links in a genome to account for recurrence and create
        a hierarchical structure which is a directed graph.

        parent_link (URDFLink) which is the parent link from which to expand.
        uniq_parent_name (str) which is a unique identifier for the parent link.
        flat_links ([URDFLink]) which is a list of URDFLink objects representing the flat genome structure.
        exp_links ([URDFLink]) which is  initially empty list that will be filled with the
        expanded URDFLink objects.
        """
    ...
    def genome_to_links(gdicts):
        """
        converts genome dictionaries to a list of URDFLink object (links)
        ...
        for gdict in gdicts:
            ...
            assert parent_ind < len(parent_names), (
                "genome.py: parent ind too high: "
                + str(parent_ind)
                + "got: " 
                + str(parent_names)
            )
    ...
    def crossover(g1, g2):
        """
        does a crossover operation between two genomes to create a new genome.

        g1 and g2 are raw DNA data (genome) - arrays of arrays of floats
        """
    ...
    def point_mutate(genome, rate, amount):
        """
        applies point mutations to a genome at a given rate and amount
        """
    ...
    def shrink_mutate(genome, rate):
        """
        removes a gene at a given rate
        """
        if len(genome) == 1:
    ...
    def grow_mutate(genome, rate):
        """
        adds new random gene at a given rate
        """
        if random.random() < rate:
    ...
    def to_csv(dna, csv_file):
        """
        writes a genome's DNA to a CSV file.
        """
        csv_str = ""
        for gene in dna:
            for val in gene:
    ...
    def from_csv(filename):
        """
        reads a genome's DNA from a csv file and returns it as a list of arrays
        """
        csv_str = ''
...
class URDFLink:
    """
    URDFLink class represents a link in the robot which is a body part of the robot.
    """
    def __init__(
            self,
            name, parent_name, recur,
            link_length=0.1,
            link_radius=0.1,
            link_mass=0.1,
            joint_type=0.1,
